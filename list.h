...
/**
 * list_replace - replace oldnode entry by newnode one
 * @oldnode : the element to be replaced
 * @newnode : the newnode element to insert
 *
 * If @oldnode was empty, it will be overwritten.
 */
 ...
 */
static inline void hlist_move_list(struct hlist_head *oldnode,
                                   struct hlist_head *newnode)
{
    newnode->first = oldnode->first;
    if (newnode->first)
    {
...
#define LIST_SIZE(type, head, member) \
    ({type * pos; int count = 0;               \
